import { defineComponent, mergeModels, useModel, computed, unref, mergeProps, withCtx, renderSlot, createVNode, createBlock, createCommentVNode, openBlock, createTextVNode, toDisplayString, readonly, ref, shallowRef, h, resolveComponent, useSlots, inject, hasInjectionContext, toRef, provide, watch, resolveDynamicComponent, toValue, isRef, shallowReactive, onServerPrefetch, getCurrentInstance, nextTick, reactive, markRaw, createElementBlock, cloneVNode, defineAsyncComponent, useSSRContext, Suspense, Fragment, createApp, renderList, useId, onErrorCaptured, effectScope, isReadonly, isShallow, isReactive, toRaw, withModifiers, getCurrentScope } from 'vue';
import { s as serialize, e as createError$1, j as parseQuery, k as defu, l as klona, m as defuFn, n as isEqual, o as hasProtocol, q as joinURL, w as withQuery, r as withTrailingSlash, t as withoutTrailingSlash, v as isScriptProtocol, x as sanitizeStatusCode, y as getContext, $ as $fetch$1, z as baseURL, A as createHooks, B as executeAsync, C as toRouteMatcher, D as createRouter$1, f as destr, E as parse, F as getRequestHeader, G as setCookie, H as getCookie, I as deleteCookie } from '../nitro/nitro.mjs';
...
import { ssrRenderComponent, ssrRenderSlot, ssrRenderClass, ssrInterpolate, ssrRenderVNode, ssrRenderList, ssrRenderAttrs, ssrRenderStyle, ssrRenderSuspense } from 'vue/server-renderer';
...
import { createSharedComposable, reactivePick, reactiveOmit, useDebounceFn } from '@vueuse/core';
...
const _routes = [
  {
    name: "index",
    path: "/",
    component: () => import('./index-BabfQiMe.mjs')
  },
  {
    name: "demo-id",
    path: "/demo/:id()",
    component: () => import('./_id_-BTeC6f-F.mjs')
  },
  {
    name: "jobs-id",
    path: "/jobs/:id()",
    component: () => import('./_id_-DxyR_Aqc.mjs'),
    children: [
      {
        name: "jobs-id-edit",
        path: "edit",
        component: () => import('./edit-CcKRFHcf.mjs')
      }
    ]
  },
  {
    name: "demo",
    path: "/demo",
    meta: { "middleware": "demo" },
    component: () => import('./index-C3Oc0c4w.mjs')
  },
  {
    name: "jobs",
    path: "/jobs",
    component: () => import('./index-C6IvJ9Hi.mjs')
  },
  {
    name: "demo-create",
    path: "/demo/create",
    meta: { "middleware": "demo" },
    component: () => import('./create-fYzQVir0.mjs')
  },
  {
    name: "jobs-create",
    path: "/jobs/create",
    component: () => import('./create-C93H2orx.mjs')
  },
  {
    name: "login",
    path: "/login",
    meta: __nuxt_page_meta || {},
    component: () => import('./index-BaKJ3YgD.mjs')
  },
  {
    name: "profile",
    path: "/profile",
    component: () => import('./index-LsRKjdQo.mjs')
  },
  {
    name: "demo-edit-id",
    path: "/demo/edit/:id()",
    meta: { "middleware": "demo" },
    component: () => import('./_id_-CLTyuAbk.mjs')
  }
];
...
function useApi() {
  ...
  function buildBaseHeaders(requiresAuth = false) {
    const base = { Accept: "application/json" };
    const auth = requiresAuth ? getAuthHeaders() : {};
    return { ...base, ...auth };
  }
  async function get2(endpoint, requiresAuth = false) {
  ...
  async function post(endpoint, data, requiresAuth = false, isFormData = false) {
    const headers = buildBaseHeaders(requiresAuth);
  ...
  async function patch(endpoint, data, requiresAuth = false) {
    const headers = buildBaseHeaders(requiresAuth);
    headers["Content-Type"] = "application/json";
    return await $fetch(`${config.public.apiBase}/${endpoint}`, {
      method: "PATCH",
  ...
  async function del(endpoint, requiresAuth = false) {
    return await $fetch(`${config.public.apiBase}/${endpoint}`, {
      method: "DELETE",
  ...
  async function me() {
    return await get2("auth/me", true);
  }
  async function refresh() {
    return await get2("auth/refresh", true);
  }
  async function tokens() {
    return await get2("auth/tokens", true);
  }
  async function getVacanciesStats() {
    return await get2("vacancies/stats", true);
  }
  async function getVacancy(id, expand) {
    const params = expand ? `?expand=${expand}` : "";
    return await get2(`vacancies/${id}${params}`, true);
  }
  ...
  const usePaginatedData = (endpoint, requiresAuth = false) => {
    const data = ref();
    ...
    const fetchData = async (params) => {
      ...
      try {
        ...
        data.value = await get2(url, requiresAuth);
      } catch (err) {
        error.value = err;
      } finally {
        pending.value = false;
...
function useAuth() {
  ...
  const isTokenExpiringSoon = (seconds = 300) => {
    const tokenData = appStore.getToken;
    if (!tokenData?.expires_at) return true;
    const now = Math.floor(Date.now() / 1e3);
  ...
  const getTokenTimeRemainingFormatted = () => {
...
const auth_45global = /* @__PURE__ */ defineNuxtRouteMiddleware(async (to, from) => {
  ...
  if (publicRoutes.includes(to.name) || to.path.startsWith("/demo")) {
    return;
  }
  const {
    isTokenExpired,
  ...
  try {
    ;
    [__temp, __restore] = executeAsync(() => getCurrentUser()), await __temp, __restore();
    ;
  } catch (error) {
    if (error?.status === 401 || error?.status === 403) {
      [__temp, __restore] = executeAsync(() => logout()), await __temp, __restore();
...
const _sfc_main$3 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "w-screen h-screen fixed inset-0 bg-white/90 backdrop-blur-sm z-50 flex items-center justify-center" }, _attrs))}><div class="text-center max-w-md mx-auto p-8"><div class="flex justify-center mb-8"><div class="relative"><div class="w-10 h-10 bg-gradient-to-r from-slate-500 to-gray-600 rounded-2xl flex items-center justify-center shadow-xl animate-pulse">`);
      _push(ssrRenderComponent(_component_UIcon, {
        name: "i-lucide-briefcase",
        class: "w-8 h-8 text-white"
      }, null, _parent));
      _push(`</div><div class="absolute -inset-2 border-2 border-slate-200 border-t-slate-500 rounded-full animate-spin"></div></div></div><h2 class="text-xl font-semibold text-gray-900 mb-2">${ssrInterpolate(_ctx.message)}${ssrInterpolate(unref(dots))}</h2><p class="text-gray-600 text-sm mb-6"> Пожалуйста, подождите </p>`);
      if (_ctx.showProgress) {
        _push(`<div class="w-full bg-gray-200 rounded-full h-1.5 mb-4"><div class="bg-gradient-to-r from-slate-500 to-gray-600 h-1.5 rounded-full transition-all duration-300" style="${ssrRenderStyle({ width: `${Math.min(100, Math.max(0, _ctx.progress))}%` })}"></div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div class="flex items-center justify-center gap-2 text-xs text-gray-500">`);
      _push(ssrRenderComponent(_component_UIcon, {
        name: "i-lucide-info",
        class: "w-3 h-3"
      }, null, _parent));
      _push(`<span>Инициализация системы</span></div></div></div>`);
    };
  }
});
...
const __nuxt_component_1 = Object.assign(_sfc_main$3, { __name: "LoadingScreen" });
...
function useLoadingScreen() {
  ...
  const incrementProgress = (step = 10) => {
    progress.value = Math.min(100, progress.value + step);
  };
  const autoHide = (delay = 2e3) => {
    setTimeout(() => {
      hide();
    }, delay);
  };
  const simulateLoading = async (steps, stepDelay = 500) => {
...
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  __ssrInlineRender: true,
  setup(__props) {
    const { isLoading, message, progress, showProgress } = useLoadingScreen();
    return (_ctx, _push, _parent, _attrs) => {
      const _component_UApp = __nuxt_component_0;
      const _component_LoadingScreen = __nuxt_component_1;
      const _component_NuxtPage = __nuxt_component_2;
      _push(ssrRenderComponent(_component_UApp, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            if (unref(isLoading)) {
              _push2(ssrRenderComponent(_component_LoadingScreen, {
            ...
            return [
              unref(isLoading) ? (openBlock(), createBlock(_component_LoadingScreen, {
                key: 0,
                message: unref(message),
                "show-progress": unref(showProgress),
                progress: unref(progress)
              }, null, 8, ["message", "show-progress", "progress"])) : createCommentVNode("", true),
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    const stack = void 0;
    const _Error404 = defineAsyncComponent(() => import('./error-404-WoSo_f5Q.mjs'));
    const _Error = defineAsyncComponent(() => import('./error-500-D0niCm28.mjs'));
...
export { _sfc_main$d as A, getDisplayValue as B, formBusInjectionKey as C, formErrorsInjectionKey as D, formInputsInjectionKey as E, formLoadingInjectionKey as F, formOptionsInjectionKey as G, inputIdInjectionKey as H, formFieldInjectionKey as I, defineNuxtRouteMiddleware as J, useLoadingScreen as K, createError as L, __nuxt_component_0$1 as _, useRouter as a, useAuth as b, _sfc_main$e as c, _sfc_main$9 as d, entry$1 as default, useRoute as e, useToast as f, useApi as g, useAppConfig as h, useLocale as i, usePortal as j, isArrayOfArray as k, _sfc_main$c as l, get as m, _sfc_main$a as n, omit as o, pickLinkProps as p, _sfc_main$b as q, useOverlay as r, useAppStore as s, tv as t, useHead as u, __nuxt_component_2$1 as v, useButtonGroup as w, useComponentIcons as x, useFormField as y, looseToNumber as z };
